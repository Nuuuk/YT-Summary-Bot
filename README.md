...
2. Edit `CHANNELS` in `summary_bot.py` with target Channel IDs (`UC...`).
3. Trigger manually in the **Actions** tab or let it run on schedule.
...
### 🚀 快速配置
1. 在 GitHub 仓库的 `Settings` > `Secrets and variables` > `Actions` 中添加配置：
   * `GEMINI_API_KEY`：Google AI Studio API 密钥。
   * `SENDER_EMAIL`：发件邮箱（如 Gmail）。
   * `SENDER_PASSWORD`：邮箱 16 位应用专用密码。
   * `RECEIVER_EMAIL`：接收报告的邮箱地址。
2. 在 `summary_bot.py` 中更新 `CHANNELS` 列表对应的 YouTube 频道 ID（以 `UC` 开头）。
3. 在 **Actions** 页面手动点击 **Run workflow** 运行测试，或等待定时任务自动触发。
